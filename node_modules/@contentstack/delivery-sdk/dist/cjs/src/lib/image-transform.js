...
class ImageTransform {
    ...
    resize({ width, height, disable, }) {
        if (width)
            this.obj.width = width.toString();
        if (height)
            this.obj.height = height.toString();
        if (disable)
            this.obj.disable = 'upscale';
        return this;
    }
    /**
     * @method crop
     ...
     * const transformURL = url.transform(transformObj);
     */
    crop({ width, height, xval, yval, cropBy = types_1.CropBy.DEFAULT, safe = false, smart = false, }) {
        switch (cropBy) {
            case types_1.CropBy.DEFAULT: {
                this.obj.crop = [width.toString(), height.toString()];
                break;
            }
            case types_1.CropBy.ASPECTRATIO: {
                this.obj.crop = `${width}:${height}`;
                break;
            }
            case types_1.CropBy.REGION: {
                this.obj.crop = [width.toString(), height.toString(), `x${xval}`, `y${yval}`];
                break;
            }
            case types_1.CropBy.OFFSET: {
                this.obj.crop = [width.toString(), height.toString(), `offset-x${xval}`, `offset-y${yval}`];
            }
        }
        if (safe)
            this.obj.crop = [...this.obj.crop, 'safe'];
        if (smart)
            this.obj.crop = [...this.obj.crop, 'smart'];
        return this;
    }
    /**
     * @method fit
    ...
    overlay({ relativeURL, align, repeat, width, height, }) {
        ...
        if (align)
            this.obj['overlay-align'] = align.toString();
        if (repeat)
            this.obj['overlay-repeat'] = repeat.toString();
        if (width)
            this.obj['overlay-width'] = width.toString();
        if (height)
            this.obj['overlay-height'] = height.toString();
        return this;
    }
    /**
     * @method padding
     ...
     * const transformURL = url.transform(transformObj);
     */
    canvas({ width, height, xval, yval, canvasBy = types_1.CanvasBy.DEFAULT, }) {
        switch (canvasBy) {
            case types_1.CanvasBy.DEFAULT: {
                this.obj.canvas = [width.toString(), height.toString()];
                break;
            }
            case types_1.CanvasBy.ASPECTRATIO: {
                this.obj.canvas = `${width}:${height}`;
                break;
            }
            case types_1.CanvasBy.REGION: {
                this.obj.canvas = [width.toString(), height.toString()];
                if (xval)
                    this.obj.canvas.push(`x${xval}`);
                if (yval)
                    this.obj.canvas.push(`y${yval}`);
                break;
            }
            case types_1.CanvasBy.OFFSET: {
                this.obj.canvas = [width.toString(), height.toString()];
                if (xval)
                    this.obj.canvas.push(`offset-x${xval}`);
                if (yval)
                    this.obj.canvas.push(`offset-y${yval}`);
            }
        }
        return this;
    }
